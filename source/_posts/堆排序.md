...
description: Sorting 
---
<p class="description">堆排序使用的是二叉堆。首先将所有的元素添加到一个对上，然后不断移除最大的元素以获得一个排好序的线性表。</p>


<!-- more -->
## 堆的存储
堆排序(heap sort)使用二叉堆(binary heap)，它是一棵完全二叉树，每个节点大于或等于它的任意一个孩子。如果一颗二叉树的每一层都是满的，或者最后一层可以不填满并且最后一层的叶子都是靠左放置的，那么这棵二叉树就是完全的(complete)。
如果堆的大小事事先知道的，那么尅讲堆存储在一个ArrayList或一个数组中。树根在位置0处，他的两个子节点在位置1和位置2出。
对于位置<i>i</i>处的节点，它的：
