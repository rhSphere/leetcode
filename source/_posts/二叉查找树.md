...
description:
---
<p class="description">二叉查找树（没有重复元素）的特征是：对于树中的每一个节点，它的左子树中的节点的值都小于该节点的值，而它的右子树中节点的值都大于该节点的值。</p>


<!-- more -->
## 表示二叉查找树
二叉查找树（Binary Search Tree, BST）可以用一个链式节点的集合来表示二叉树。每个节点都包含一个数值和两个称为left和right的链接，分别指向左孩子和右孩子。
```java 树的节点类
...
对二叉树进行前序、中序和后序遍历时都开始于根节点或结束于根节点，经由路线也相同。彼此差别在于对节点访问时机的选择不同。3中遍历方式都是沿着左子树不断深入下去，当到达二叉树左下节点而无法往下深入时，就向上逐一返回，行进到最近深入时曾近遇到节点的右子树，然后进行同样的深入和返回，直到最终从根节点的右子树返回到根节点。这样，遍历时返回顺序与深入节点顺序恰好相反，因此可以再实现二叉树遍历过程中使用一个工作栈来保存当前深入到的节点信息，一共后面返回需要时使用。

### 中序遍历(inorder traversal)
1+2  可以递增顺序显示BST中所有节点。
- 中序遍历访问左子二叉树
- 访问根节点
- 中序遍历访问右子二叉树
...
## 查找一个元素 
二叉查找树中查找一个元素，可以从根节点向下扫描，知道找到匹配元素，或者达到一棵空子树为止。
```java 在BST中查找一个元素
...
## 删除BST中的一个元素
为了从一棵二叉查找树中删除一个元素，首先需要定位该元素位置，然后再删除该元素以及重新连接树前，考虑两种情况--该节点有或者没有左子节点。
...
```java 从BST中删除一个元素
    public boolean delete(E e) {
        ...
        TreeNode<E> current = root; // 指向二叉查找树中包含该元素的节点
